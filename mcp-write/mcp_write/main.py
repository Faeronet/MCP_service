...
def _parse_fizicheskie_daty_ddmm(val: str) -> list[str]:
    """Из значения ключа «Физическое» извлечь даты «число месяц_р_п» → нормализованные строки dd.mm."""
    ...
    # Сортировка по (месяц, день) в году без года
...
def _ingest_document_system_b(req: IngestDocumentRequest, raw: str) -> dict[str, Any]:
    ...
    if phys_val:
        ddmm = _parse_fizicheskie_daty_ddmm(phys_val)
        _save_angel_physical_dates_postgres(main_chunk_id, req.doc_id, name or "", ddmm)
    else:
        _save_angel_physical_dates_postgres(main_chunk_id, req.doc_id, name or "", [])
