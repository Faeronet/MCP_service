...
- **Эмбеддинг:** mcp-read и mcp-write используют **заглушку** (нулевой вектор или один и тот же вектор), поэтому поиск по смыслу не работает — в Qdrant одна коллекция **chunks**, поиск идёт по вектору, но векторы не семантические. Чтобы включить эмбеддинг, нужно подключить вызов API (vLLM embeddings или отдельный сервис) в mcp-read (build_context) и в mcp-write (ingest).
...
- **Консоль:** `http://IP_СЕРВЕРА:9001` (логин/пароль — `MINIO_ACCESS_KEY` / `MINIO_SECRET_KEY` из `.env`)

Если консоль при открытии по IP редиректит на localhost, задайте в `.env`:
```bash
MINIO_SERVER_URL=http://IP_СЕРВЕРА:9001
```
и перезапустите MinIO: `docker compose up -d minio`.
