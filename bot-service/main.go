...
// Извлечение сущностей только через Prose: doc.Entities() + разбор doc.Tokens() по триггерам и словарям.
...
// Национальности и группы: все падежи и варианты для поиска вхождений в вопросе.
var nationalityWords = []string{
	"евреи", "евреев", "евреям", "евреями", "евреях", "еврей", "евреем",
	"армяне", "армян", "армянам", "армянами", "армянах", "армянин", "армянина", "армянка",
	"грузины", "грузин", "грузинам", "грузинами", "грузинка",
	"русские", "русских", "русским", "русскими", "русский", "русская",
	"украинцы", "украинцев", "украинцам", "украинцев", "украинец", "украинка",
	"татары", "татар", "татарам", "татарами", "татарин", "татарка",
	"греки", "греков", "грекам", "греками", "грек", "гречанка",
	"турки", "турок", "туркам", "турками", "турок", "турчанка",
	"поляки", "поляков", "полякам", "поляками", "поляк", "полячка",
	"немцы", "немцев", "немцам", "немцами", "немец", "немка",
	"французы", "французов", "французам", "французами", "француз", "француженка",
	"итальянцы", "итальянцев", "итальянцам", "итальянцами", "итальянец", "итальянка",
	"испанцы", "испанцев", "испанцам", "испанцами", "испанец", "испанка",
	"англичане", "англичан", "англичанам", "англичанами", "англичанин", "англичанка",
	"китайцы", "китайцев", "китайцам", "китайцами", "китаец", "китаянка",
	"японцы", "японцев", "японцам", "японцами", "японец", "японка",
	"индийцы", "индийцев", "индийцам", "индийцами", "индиец", "индианка",
	"арабы", "арабов", "арабам", "арабами", "араб",
}
...
// Проблемы и состояния: сон, бессонница, стресс и т.д. (для вопросов «ангел от бессонницы», «помогает при сне»).
var problemWords = []string{
	"сон", "сна", "сну", "сном", "сне", "бессонница", "бессонницу", "бессонницей", "бессоннице",
	"стресс", "стресса", "стрессу", "стрессом", "стрессе",
	"депрессия", "депрессии", "депрессию", "депрессией",
	"тревога", "тревоги", "тревогу", "тревогой", "тревоге",
	"страх", "страха", "страху", "страхом", "страхе", "страхи",
	"усталость", "усталости", "усталостью",
	"головная боль", "головной боли", "мигрень", "мигрени", "мигренью",
	"беспокойство", "беспокойства", "беспокойству", "беспокойством",
	"апатия", "апатии", "апатию", "апатией",
	"бессоннице", "бессонницы",
}
...
// Словари как множества для поиска по токенам Prose (нижний регистр).
var (
	bodyPartSet       = makeSet(bodyPartWords)
	nationalitySet    = makeSet(nationalityWords)
	zodiacSet         = makeSet(zodiacWords)
	problemSet        = makeSet(problemWords)
	monthNamesForDate = makeSet(monthVariantsForTokens)
)
...
// extractSearchEntitiesFromQuestion извлекает сущности только через Prose: Entities() + разбор Tokens() по словарям и триггерам.
func extractSearchEntitiesFromQuestion(question string) string {
	...
	// Фразы после триггеров (исцелить X, влияет на X, властвует над X, помогает при X, от X)
	...
	// Одиночные токены по словарям: части тела, национальности, знаки зодиака, проблемы
	for _, tok := range tokens {
		tl := strings.ToLower(tok.Text)
		if _, ok := bodyPartSet[tl]; ok {
			add(tok.Text)
		}
		if _, ok := nationalitySet[tl]; ok {
			add(tok.Text)
		}
		if _, ok := zodiacSet[tl]; ok {
			add(tok.Text)
		}
		if _, ok := problemSet[tl]; ok {
			add(tok.Text)
		}
	}
