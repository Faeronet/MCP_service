...
func main() {
	...
	if raw, err := promptFS.ReadFile("prompts/reminder_compose.txt"); err == nil {
		app.promptC = strings.TrimSpace(string(raw))
	}
	if app.promptA == "" {
	...
	if app.promptC == "" {
		app.promptC = "Кратко напомни пользователю об ангеле по контексту. Один–три коротких предложения."
	}

	schedCtx, schedCancel := context.WithCancel(context.Background())
	defer schedCancel()
	go app.reminderSchedulerLoop(schedCtx)
	go func() {
		time.Sleep(3 * time.Second)
		app.reconcileRemindersOnStartup(context.Background())
	}()
...
type Bot struct {
	...
	promptC        string // напоминание об ангеле (LLM)
...
func (b *Bot) processMessage(ctx context.Context, u tgbotapi.Update, chatID int64, userID int64, requestID string) {
	...
	if contextText == "" {
		...
		llmLine, errLLM := b.extractSearchQuery(ctx, requestID, msg.Text)
		stripped := strings.TrimSpace(llmLine)
		hadReminder := false
		if hh, mm, ok := parseReminderLine(stripped); ok {
			hadReminder = true
			if err := b.upsertReminderSubscriber(ctx, userID, chatID, hh, mm); err != nil {
				log.Warn(ctx, "reminder subscribe", logging.KV{"error", err})
			} else {
				b.sendTodayAngelReminderToChat(ctx, requestID, chatID)
			}
		}
		if hadReminder {
			searchQuery = b.deterministicSearchQuery(msg.Text)
		} else if errLLM != nil || stripped == "" || strings.EqualFold(stripped, "null") {
			searchQuery = b.deterministicSearchQuery(msg.Text)
		} else {
		...
		case "[date] list":
			...
			var err error
			contextText, _, _, _, err = b.buildContext(ctx, requestID, searchQuery, attachmentsText, 4000, "default")
			if err != nil {
	...
	// Без vLLM: ответ = найденный контекст.
	reply := strings.TrimSpace(contextText)
	if reply == "" {
...
// deterministicSearchQuery формирует поисковую строку без LLM (как раньше в processMessage).
func (b *Bot) deterministicSearchQuery(msgText string) string {
	translatedMsg := translateMonthToRussian(msgText)
	userDateStr := extractDateFromQuestion(translatedMsg)
	var searchQuery string
	if userDateStr != "" {
		searchQuery = userDateStr
	} else {
		searchQuery = translatedMsg
	}
	if hasCollectionTriggerWords(msgText) && !hasCollectionTriggerWords(searchQuery) {
		searchQuery = translatedMsg
	}
	lowerMsg := strings.ToLower(strings.TrimSpace(msgText))
	if hasAngelWord(msgText) {
...
func (b *Bot) extractSearchQuery(ctx context.Context, requestID, userQuestion string) (string, error) {
	if strings.TrimSpace(b.vllmBase) == "" || strings.TrimSpace(b.promptA) == "" {
		return "", fmt.Errorf("llm not configured")
	}
	reply, err := b.callLLMMax(ctx, requestID, b.promptA, userQuestion, 128)
	if err != nil {
		return "", err
	}
...
func (b *Bot) callLLM(ctx context.Context, requestID, systemContent, userQuery string) (string, error) {
	return b.callLLMMax(ctx, requestID, systemContent, userQuery, 512)
}

func (b *Bot) callLLMMax(ctx context.Context, requestID, systemContent, userQuery string, maxTokens int) (string, error) {
	...
	payload, _ := json.Marshal(map[string]interface{}{
		"model": b.llmModel, "messages": messages, "max_tokens": maxTokens,
...
func (b *Bot) handleAttachment(ctx context.Context, u tgbotapi.Update, chatID int64, requestID string) {
	...
	if contextText == "" {
		var searchQuery string
		// Без vLLM: детерминированно формируем поисковый запрос из текста вложения.
		translatedMsg := translateMonthToRussian(userMsg)
		userDateStr := extractDateFromQuestion(translatedMsg)
		if userDateStr != "" {
			searchQuery = userDateStr
		} else {
		...
		// Если есть коллекционные триггеры, но мы сузили query до даты — восстанавливаем исходник для роутинга.
	...
	// Без vLLM: ответ = найденный контекст (если он пуст — пробуем показать извлеченный текст).
	reply := strings.TrimSpace(contextText)
	if reply == "" && len(extracted) > 0 {
		reply = strings.TrimSpace(extracted)
	}
	msgID, _ := b.appendMessage(ctx, sessionID, "assistant", reply)
	b.trimSessionMessagesIfNeeded(ctx, sessionID)
